...
1. **Preprocessing** — `python preprocessing/preprocess_train.py` loads the
   raw PKU-SafeRLHF, Anthropic HH-RLHF, BeaverTails, and WildGuardMix datasets
...
2. **GPT annotation** (insert filename here) : a GPT-based
   annotation script run on `preprocessed.jsonl` that labels each example's
   prompt and response safety and produces `annotated_train.xlsx` using API calls.

3. **LoRA / full fine-tuning**: train your adapters/checkpoints against
   `annotated_train.xlsx` (or its derived training file).
...
4. **Capabilities evaluation**: once you have base models, LoRA adapters,
   ...
   cached. This runs IFEval + MMLU across the base model, full fine-tune, and
   every LoRA rank/placement condition, for all three model
