"""
Run IFEval + MMLU across the base Falcon-7B model and all LoRA
...
# ---- Paths (edit if your scratch path differs) ----
BASE_MODEL = "/scratch/f42827en/base_models/falcon7b"
LORA_ROOT = Path("/scratch/f42827en/lora_models")
FULL_FT_MODEL = Path("/scratch/f42827en/fullSFT_models/falcon_full")
...
def main():
    ...
    if FULL_FT_MODEL.is_dir() and (FULL_FT_MODEL / "config.json").exists():
        run_eval(
            ...
            tag="falcon7b_full_ft (ifeval+mmlu)",
        )
    else:
    ...
    for rank in RANKS:
        for placement in PLACEMENTS:
            ...
            adapter_dir = LORA_ROOT / f"falcon7b_{condition}"
            ...
            out_path = OUTPUT_ROOT / f"falcon7b_{condition}"

            run_eval(model_args=model_args, out_path=out_path, tag=f"falcon7b_{condition}")
