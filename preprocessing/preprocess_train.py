...
# Paraphrase / near-duplicate removal settings
...
before = len(df)
df = df[~df["prompt"].str.strip().isin(test_prompts)].reset_index(drop=True)
after = len(df)

print(f"Removed {before - after} overlapping prompts from training data")
print(f"Training data size: {after}", flush=True)
...
print(f"Removing near-duplicates at threshold {PARAPHRASE_THRESHOLD}...", flush=True)
...
for i in tqdm(range(len(embeddings))):
    ...
    D, I = index.search(query, k=min(50, len(keep)))

    is_duplicate = False
    for dist, idx in zip(D[0], I[0]):
        if idx == i:
            continue
        if idx in set(keep) and dist >= PARAPHRASE_THRESHOLD:
            is_duplicate = True
            break
...
print(f"\nRemoved {removed} near-duplicates", flush=True)
print(f"Final dataset size: {len(keep)}", flush=True)

# Spot check
removed_indices = [i for i in range(len(data)) if i not in set(keep)]
print("\nSpot check — sample of removed near-duplicates:")
for idx in removed_indices[:5]:
    print(f"\n  Removed: {data[idx]['prompt'][:150]}")

final_data = [data[i] for i in keep]
...
print(f"\nSaved {len(final_data)} examples to {OUTPUT_FILE}", flush=True)
